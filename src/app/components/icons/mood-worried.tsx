const MoodWorried = () => {
    return (
        <svg
            className="size-8"
            xmlns="http://www.w3.org/2000/svg"
            fill="none"
            viewBox="0 0 32 32"
        >
            <g clipPath="url(#worried-a)">
                <path
                    fill="url(#worried-b)"
                    d="M15.999 29.998c9.334 0 13.999-6.268 13.999-14 0-7.73-4.665-13.998-14-13.998C6.665 2 2 8.268 2 15.999c0 7.731 4.664 13.999 13.999 13.999z"
                ></path>
                <path
                    fill="url(#worried-c)"
                    d="M15.999 29.998c9.334 0 13.999-6.268 13.999-14 0-7.73-4.665-13.998-14-13.998C6.665 2 2 8.268 2 15.999c0 7.731 4.664 13.999 13.999 13.999z"
                ></path>
                <path
                    fill="url(#worried-d)"
                    d="M15.999 29.998c9.334 0 13.999-6.268 13.999-14 0-7.73-4.665-13.998-14-13.998C6.665 2 2 8.268 2 15.999c0 7.731 4.664 13.999 13.999 13.999z"
                ></path>
                <path
                    fill="url(#worried-e)"
                    fillOpacity="0.6"
                    d="M15.999 29.998c9.334 0 13.999-6.268 13.999-14 0-7.73-4.665-13.998-14-13.998C6.665 2 2 8.268 2 15.999c0 7.731 4.664 13.999 13.999 13.999z"
                ></path>
                <path
                    fill="url(#worried-f)"
                    d="M15.999 29.998c9.334 0 13.999-6.268 13.999-14 0-7.73-4.665-13.998-14-13.998C6.665 2 2 8.268 2 15.999c0 7.731 4.664 13.999 13.999 13.999z"
                ></path>
                <path
                    fill="url(#worried-g)"
                    d="M15.999 29.998c9.334 0 13.999-6.268 13.999-14 0-7.73-4.665-13.998-14-13.998C6.665 2 2 8.268 2 15.999c0 7.731 4.664 13.999 13.999 13.999z"
                ></path>
                <path
                    fill="url(#worried-h)"
                    d="M15.999 29.998c9.334 0 13.999-6.268 13.999-14 0-7.73-4.665-13.998-14-13.998C6.665 2 2 8.268 2 15.999c0 7.731 4.664 13.999 13.999 13.999z"
                ></path>
                <path
                    fill="url(#worried-i)"
                    d="M15.999 29.998c9.334 0 13.999-6.268 13.999-14 0-7.73-4.665-13.998-14-13.998C6.665 2 2 8.268 2 15.999c0 7.731 4.664 13.999 13.999 13.999z"
                ></path>
                <circle cx="9" cy="16" r="5" fill="url(#j)"></circle>
                <circle cx="19.5" cy="16.5" r="4.5" fill="url(#k)"></circle>
                <path
                    fill="#fff"
                    d="M10.5 19a4.5 4.5 0 100-9 4.5 4.5 0 000 9zm11 0a4.5 4.5 0 100-9 4.5 4.5 0 000 9z"
                ></path>
                <path
                    fill="url(#worried-l)"
                    d="M12 17a2 2 0 100-4 2 2 0 000 4z"
                ></path>
                <path
                    fill="url(#worried-m)"
                    d="M20 17a2 2 0 100-4 2 2 0 000 4z"
                ></path>
                <path
                    stroke="url(#worried-n)"
                    strokeLinecap="round"
                    strokeLinejoin="round"
                    strokeWidth="1.25"
                    d="M10.5 5.5s-.5 3-4 3"
                ></path>
                <path
                    stroke="url(#worried-o)"
                    strokeLinecap="round"
                    strokeLinejoin="round"
                    strokeWidth="1.25"
                    d="M10.5 5.5s-.5 3-4 3"
                ></path>
                <path
                    stroke="url(#worried-p)"
                    strokeLinecap="round"
                    strokeLinejoin="round"
                    strokeWidth="1.25"
                    d="M21.5 5.5s.5 3 4 3"
                ></path>
                <path
                    stroke="url(#worried-q)"
                    strokeLinecap="round"
                    strokeWidth="2"
                    d="M12 25c.333-.667 1.6-2 4-2s3.667 1.333 4 2"
                ></path>
            </g>
            <defs>
                <radialGradient
                    id="worried-b"
                    cx="0"
                    cy="0"
                    r="1"
                    gradientTransform="rotate(132.839 10.786 10.065) scale(37.5033)"
                    gradientUnits="userSpaceOnUse"
                >
                    <stop stopColor="#FFF478"></stop>
                    <stop offset="0.475" stopColor="#FFB02E"></stop>
                    <stop offset="1" stopColor="#F70A8D"></stop>
                </radialGradient>
                <radialGradient
                    id="worried-c"
                    cx="0"
                    cy="0"
                    r="1"
                    gradientTransform="rotate(131.878 10.74 10.193) scale(38.9487)"
                    gradientUnits="userSpaceOnUse"
                >
                    <stop stopColor="#FFF478"></stop>
                    <stop offset="0.475" stopColor="#FFB02E"></stop>
                    <stop offset="1" stopColor="#F70A8D"></stop>
                </radialGradient>
                <radialGradient
                    id="worried-d"
                    cx="0"
                    cy="0"
                    r="1"
                    gradientTransform="rotate(101.31 2.876 12.808) scale(17.8466 22.8581)"
                    gradientUnits="userSpaceOnUse"
                >
                    <stop
                        offset="0.788"
                        stopColor="#F59639"
                        stopOpacity="0"
                    ></stop>
                    <stop offset="0.973" stopColor="#FF7DCE"></stop>
                </radialGradient>
                <radialGradient
                    id="worried-e"
                    cx="0"
                    cy="0"
                    r="1"
                    gradientTransform="matrix(-29 29 -29 -29 18 14)"
                    gradientUnits="userSpaceOnUse"
                >
                    <stop offset="0.315" stopOpacity="0"></stop>
                    <stop offset="1"></stop>
                </radialGradient>
                <radialGradient
                    id="worried-f"
                    cx="0"
                    cy="0"
                    r="1"
                    gradientTransform="rotate(77.692 -2.555 18.434) scale(28.1469)"
                    gradientUnits="userSpaceOnUse"
                >
                    <stop
                        offset="0.508"
                        stopColor="#7D6133"
                        stopOpacity="0"
                    ></stop>
                    <stop offset="1" stopColor="#715B32"></stop>
                </radialGradient>
                <radialGradient
                    id="worried-g"
                    cx="0"
                    cy="0"
                    r="1"
                    gradientTransform="matrix(7.5 10.99996 -7.97335 5.4364 16.5 16.5)"
                    gradientUnits="userSpaceOnUse"
                >
                    <stop stopColor="#FFB849"></stop>
                    <stop offset="1" stopColor="#FFB847" stopOpacity="0"></stop>
                </radialGradient>
                <radialGradient
                    id="worried-h"
                    cx="0"
                    cy="0"
                    r="1"
                    gradientTransform="matrix(11.49998 2 -2 11.49998 20.5 18)"
                    gradientUnits="userSpaceOnUse"
                >
                    <stop stopColor="#FFA64B"></stop>
                    <stop
                        offset="0.9"
                        stopColor="#FFAE46"
                        stopOpacity="0"
                    ></stop>
                </radialGradient>
                <radialGradient
                    id="worried-i"
                    cx="0"
                    cy="0"
                    r="1"
                    gradientTransform="rotate(43.971 -9.827 29.173) scale(59.0529)"
                    gradientUnits="userSpaceOnUse"
                >
                    <stop offset="0.185" stopOpacity="0"></stop>
                    <stop offset="1" stopOpacity="0.4"></stop>
                </radialGradient>
                <radialGradient
                    id="worried-j"
                    cx="0"
                    cy="0"
                    r="1"
                    gradientTransform="rotate(135 3.936 8.725) scale(9.74236 5.04232)"
                    gradientUnits="userSpaceOnUse"
                >
                    <stop stopColor="#392108"></stop>
                    <stop offset="1" stopColor="#C87928" stopOpacity="0"></stop>
                </radialGradient>
                <radialGradient
                    id="worried-k"
                    cx="0"
                    cy="0"
                    r="1"
                    gradientTransform="matrix(-5.75 5.75 -3.24597 -3.24597 23 13)"
                    gradientUnits="userSpaceOnUse"
                >
                    <stop stopColor="#392108"></stop>
                    <stop offset="1" stopColor="#C87928" stopOpacity="0"></stop>
                </radialGradient>
                <radialGradient
                    id="worried-o"
                    cx="0"
                    cy="0"
                    r="1"
                    gradientTransform="matrix(2.5 3.5 -3.5 2.5 7 5)"
                    gradientUnits="userSpaceOnUse"
                >
                    <stop offset="0.542" stopColor="#493032"></stop>
                    <stop offset="0.807" stopColor="#5E445A"></stop>
                    <stop offset="0.955" stopColor="#4A2C42"></stop>
                </radialGradient>
                <radialGradient
                    id="worried-p"
                    cx="0"
                    cy="0"
                    r="1"
                    gradientTransform="rotate(125.538 11.213 8.933) scale(4.30116)"
                    gradientUnits="userSpaceOnUse"
                >
                    <stop stopColor="#C2B4B5"></stop>
                    <stop offset="0.887" stopColor="#462B45"></stop>
                    <stop offset="1" stopColor="#4A2C42"></stop>
                </radialGradient>
                <linearGradient
                    id="worried-l"
                    x1="20.615"
                    x2="19.927"
                    y1="13"
                    y2="17.03"
                    gradientUnits="userSpaceOnUse"
                >
                    <stop stopColor="#553B3E"></stop>
                    <stop offset="1" stopColor="#3D2432"></stop>
                </linearGradient>
                <linearGradient
                    id="worried-m"
                    x1="20.615"
                    x2="19.927"
                    y1="13"
                    y2="17.03"
                    gradientUnits="userSpaceOnUse"
                >
                    <stop stopColor="#553B3E"></stop>
                    <stop offset="1" stopColor="#3D2432"></stop>
                </linearGradient>
                <linearGradient
                    id="worried-n"
                    x1="9.5"
                    x2="9.5"
                    y1="5"
                    y2="6.5"
                    gradientUnits="userSpaceOnUse"
                >
                    <stop offset="0.03" stopColor="#524049"></stop>
                    <stop offset="1" stopColor="#4A2C42"></stop>
                </linearGradient>
                <linearGradient
                    id="worried-q"
                    x1="16"
                    x2="16"
                    y1="22"
                    y2="24"
                    gradientUnits="userSpaceOnUse"
                >
                    <stop stopColor="#4F3C43"></stop>
                    <stop offset="1" stopColor="#512756"></stop>
                </linearGradient>
                <clipPath id="a">
                    <path fill="#fff" d="M0 0h32v32H0z"></path>
                </clipPath>
            </defs>
        </svg>
    );
};

export { MoodWorried };
