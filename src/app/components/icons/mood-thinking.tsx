const MoodThinking = () => {
    return (
        <svg
            className="size-24"
            xmlns="http://www.w3.org/2000/svg"
            fill="none"
            viewBox="0 0 32 32"
        >
            <path
                fill="url(#thinking-a)"
                d="M15.999 29.998c9.334 0 13.999-6.268 13.999-14 0-7.73-4.665-13.998-14-13.998C6.665 2 2 8.268 2 15.999c0 7.731 4.664 13.999 13.999 13.999z"
            ></path>
            <path
                fill="url(#thinking-b)"
                d="M15.999 29.998c9.334 0 13.999-6.268 13.999-14 0-7.73-4.665-13.998-14-13.998C6.665 2 2 8.268 2 15.999c0 7.731 4.664 13.999 13.999 13.999z"
            ></path>
            <path
                fill="url(#thinking-c)"
                d="M15.999 29.998c9.334 0 13.999-6.268 13.999-14 0-7.73-4.665-13.998-14-13.998C6.665 2 2 8.268 2 15.999c0 7.731 4.664 13.999 13.999 13.999z"
            ></path>
            <path
                fill="url(#thinking-d)"
                fillOpacity="0.6"
                d="M15.999 29.998c9.334 0 13.999-6.268 13.999-14 0-7.73-4.665-13.998-14-13.998C6.665 2 2 8.268 2 15.999c0 7.731 4.664 13.999 13.999 13.999z"
            ></path>
            <path
                fill="url(#thinking-e)"
                d="M15.999 29.998c9.334 0 13.999-6.268 13.999-14 0-7.73-4.665-13.998-14-13.998C6.665 2 2 8.268 2 15.999c0 7.731 4.664 13.999 13.999 13.999z"
            ></path>
            <path
                fill="url(#thinking-f)"
                d="M15.999 29.998c9.334 0 13.999-6.268 13.999-14 0-7.73-4.665-13.998-14-13.998C6.665 2 2 8.268 2 15.999c0 7.731 4.664 13.999 13.999 13.999z"
            ></path>
            <path
                fill="url(#thinking-g)"
                d="M15.999 29.998c9.334 0 13.999-6.268 13.999-14 0-7.73-4.665-13.998-14-13.998C6.665 2 2 8.268 2 15.999c0 7.731 4.664 13.999 13.999 13.999z"
            ></path>
            <path
                fill="url(#thinking-h)"
                d="M15.999 29.998c9.334 0 13.999-6.268 13.999-14 0-7.73-4.665-13.998-14-13.998C6.665 2 2 8.268 2 15.999c0 7.731 4.664 13.999 13.999 13.999z"
            ></path>
            <path
                fill="url(#thinking-i)"
                d="M10 21.5v-3.06A1.44 1.44 0 008.56 17c-.84 0-1.505.718-1.517 1.559C7.023 19.92 6.835 21.722 6 22c-3 1-4 5.5-1.5 7.5 2 1.6 4 1.5 5.5 1.5h2.764a1.236 1.236 0 00.553-2.342L13 28.5h.72a1 1 0 00.97-.758l.067-.272A.78.78 0 0014 26.5l.33-.165a1.214 1.214 0 000-2.17L14 24h1.75a1.25 1.25 0 100-2.5H10z"
            ></path>
            <path
                stroke="#402A32"
                strokeLinecap="round"
                strokeLinejoin="round"
                strokeWidth="2"
                d="M13 19s1.124-1.303 3.25-1c2.126.303 2.75 2 2.75 2"
            ></path>
            <circle
                cx="9.017"
                cy="13.421"
                r="4.673"
                fill="url(#thinking-j)"
            ></circle>
            <circle
                cx="19.244"
                cy="13.943"
                r="4.244"
                fill="url(#thinking-k)"
            ></circle>
            <path
                fill="#fff"
                d="M10.42 16.224a4.206 4.206 0 100-8.411 4.206 4.206 0 000 8.411zm11.148.077a4.244 4.244 0 100-8.489 4.244 4.244 0 000 8.49z"
            ></path>
            <path
                fill="url(#thinking-l)"
                fillRule="evenodd"
                d="M13 11.5a3 3 0 11-6 0 3 3 0 016 0zm11 0a3 3 0 11-6 0 3 3 0 016 0z"
                clipRule="evenodd"
            ></path>
            <g filter="url(#thinking-m)">
                <path
                    fill="#E4694E"
                    fillRule="evenodd"
                    d="M12.703 28.688l.055-.218A.78.78 0 0012 27.5l.33-.165a1.214 1.214 0 000-2.17L12 25h1.75a1.25 1.25 0 000-2.5H8v-3.06A1.44 1.44 0 006.56 18c-.84 0-1.505.718-1.517 1.559-.015 1.058-.132 2.381-.575 3.05 1.537 3.041 4.282 5.287 8.235 6.08z"
                    clipRule="evenodd"
                ></path>
            </g>
            <path
                fill="url(#thinking-n)"
                d="M10 21.5v-3.06A1.44 1.44 0 008.56 17c-.84 0-1.505.718-1.517 1.559C7.023 19.92 6.835 21.722 6 22c-3 1-4 5.5-1.5 7.5 2 1.6 4 1.5 5.5 1.5h2.764a1.236 1.236 0 00.553-2.342L13 28.5h.72a1 1 0 00.97-.758l.067-.272A.78.78 0 0014 26.5l.33-.165a1.214 1.214 0 000-2.17L14 24h1.75a1.25 1.25 0 100-2.5H10z"
            ></path>
            <path
                fill="url(#thinking-o)"
                d="M10 21.5v-3.06A1.44 1.44 0 008.56 17c-.84 0-1.505.718-1.517 1.559C7.023 19.92 6.835 21.722 6 22c-3 1-4 5.5-1.5 7.5 2 1.6 4 1.5 5.5 1.5h2.764a1.236 1.236 0 00.553-2.342L13 28.5h.72a1 1 0 00.97-.758l.067-.272A.78.78 0 0014 26.5l.33-.165a1.214 1.214 0 000-2.17L14 24h1.75a1.25 1.25 0 100-2.5H10z"
            ></path>
            <path
                stroke="#402A32"
                strokeLinecap="round"
                strokeLinejoin="round"
                d="M12.5 4.5c-.333-.333-1.3-1-2.5-1s-2.167.667-2.5 1m11 4s1-1 3-1 3 1 3 1"
            ></path>
            <g filter="url(#thinking-p)" opacity="0.8">
                <path fill="#FFA048" d="M10 23h7v1h-7z"></path>
            </g>
            <path fill="url(#thinking-q)" d="M11 24h3a1 1 0 011 1l-4-1z"></path>
            <path
                fill="url(#thinking-r)"
                d="M11 28.5h2a1 1 0 011 1l-3-1z"
            ></path>
            <path
                fill="url(#thinking-s)"
                d="M11 26.5h3a1 1 0 011 1l-4-1z"
            ></path>
            <defs>
                <radialGradient
                    id="thinking-a"
                    cx="0"
                    cy="0"
                    r="1"
                    gradientTransform="rotate(132.839 10.786 10.065) scale(37.5033)"
                    gradientUnits="userSpaceOnUse"
                >
                    <stop stopColor="#FFF478"></stop>
                    <stop offset="0.475" stopColor="#FFB02E"></stop>
                    <stop offset="1" stopColor="#F70A8D"></stop>
                </radialGradient>
                <radialGradient
                    id="thinking-b"
                    cx="0"
                    cy="0"
                    r="1"
                    gradientTransform="rotate(131.878 10.74 10.193) scale(38.9487)"
                    gradientUnits="userSpaceOnUse"
                >
                    <stop stopColor="#FFF478"></stop>
                    <stop offset="0.475" stopColor="#FFB02E"></stop>
                    <stop offset="1" stopColor="#F70A8D"></stop>
                </radialGradient>
                <radialGradient
                    id="thinking-c"
                    cx="0"
                    cy="0"
                    r="1"
                    gradientTransform="rotate(101.31 2.876 12.808) scale(17.8466 22.8581)"
                    gradientUnits="userSpaceOnUse"
                >
                    <stop
                        offset="0.788"
                        stopColor="#F59639"
                        stopOpacity="0"
                    ></stop>
                    <stop offset="0.973" stopColor="#FF7DCE"></stop>
                </radialGradient>
                <radialGradient
                    id="thinking-d"
                    cx="0"
                    cy="0"
                    r="1"
                    gradientTransform="matrix(-29 29 -29 -29 18 14)"
                    gradientUnits="userSpaceOnUse"
                >
                    <stop offset="0.315" stopOpacity="0"></stop>
                    <stop offset="1"></stop>
                </radialGradient>
                <radialGradient
                    id="thinking-e"
                    cx="0"
                    cy="0"
                    r="1"
                    gradientTransform="rotate(77.692 -2.555 18.434) scale(28.1469)"
                    gradientUnits="userSpaceOnUse"
                >
                    <stop
                        offset="0.508"
                        stopColor="#7D6133"
                        stopOpacity="0"
                    ></stop>
                    <stop offset="1" stopColor="#715B32"></stop>
                </radialGradient>
                <radialGradient
                    id="thinking-f"
                    cx="0"
                    cy="0"
                    r="1"
                    gradientTransform="matrix(7.5 10.99996 -7.97335 5.4364 16.5 16.5)"
                    gradientUnits="userSpaceOnUse"
                >
                    <stop stopColor="#FFB849"></stop>
                    <stop offset="1" stopColor="#FFB847" stopOpacity="0"></stop>
                </radialGradient>
                <radialGradient
                    id="thinking-g"
                    cx="0"
                    cy="0"
                    r="1"
                    gradientTransform="matrix(11.49998 2 -2 11.49998 20.5 18)"
                    gradientUnits="userSpaceOnUse"
                >
                    <stop stopColor="#FFA64B"></stop>
                    <stop
                        offset="0.9"
                        stopColor="#FFAE46"
                        stopOpacity="0"
                    ></stop>
                </radialGradient>
                <radialGradient
                    id="thinking-h"
                    cx="0"
                    cy="0"
                    r="1"
                    gradientTransform="rotate(43.971 -9.827 29.173) scale(59.0529)"
                    gradientUnits="userSpaceOnUse"
                >
                    <stop offset="0.185" stopOpacity="0"></stop>
                    <stop offset="1" stopOpacity="0.4"></stop>
                </radialGradient>
                <radialGradient
                    id="thinking-i"
                    cx="0"
                    cy="0"
                    r="1"
                    gradientTransform="matrix(-16.50002 -.50009 .46634 -15.38663 15.5 22)"
                    gradientUnits="userSpaceOnUse"
                >
                    <stop offset="0.066" stopColor="#FFEA67"></stop>
                    <stop offset="0.593" stopColor="#FFC13F"></stop>
                    <stop offset="0.904" stopColor="#DF9030"></stop>
                </radialGradient>
                <radialGradient
                    id="thinking-j"
                    cx="0"
                    cy="0"
                    r="1"
                    gradientTransform="rotate(135 4.3 7.513) scale(10.2026 5.28051)"
                    gradientUnits="userSpaceOnUse"
                >
                    <stop stopColor="#392108"></stop>
                    <stop offset="1" stopColor="#C87928" stopOpacity="0"></stop>
                </radialGradient>
                <radialGradient
                    id="thinking-k"
                    cx="0"
                    cy="0"
                    r="1"
                    gradientTransform="rotate(133.664 8.996 10.145) scale(9.48022 5.35173)"
                    gradientUnits="userSpaceOnUse"
                >
                    <stop stopColor="#392108"></stop>
                    <stop offset="1" stopColor="#C87928" stopOpacity="0"></stop>
                </radialGradient>
                <radialGradient
                    id="thinking-n"
                    cx="0"
                    cy="0"
                    r="1"
                    gradientTransform="matrix(-16.50002 -.50009 .46634 -15.38663 15.5 22)"
                    gradientUnits="userSpaceOnUse"
                >
                    <stop offset="0.066" stopColor="#FFEA67"></stop>
                    <stop offset="0.593" stopColor="#FFC13F"></stop>
                    <stop offset="0.904" stopColor="#DF9030"></stop>
                </radialGradient>
                <radialGradient
                    id="thinking-o"
                    cx="0"
                    cy="0"
                    r="1"
                    gradientTransform="matrix(-11 1.49997 -1.53804 -11.27922 10.5 22.5)"
                    gradientUnits="userSpaceOnUse"
                >
                    <stop
                        offset="0.5"
                        stopColor="#FFEA67"
                        stopOpacity="0"
                    ></stop>
                    <stop offset="0.851" stopColor="#F9708E"></stop>
                </radialGradient>
                <linearGradient
                    id="thinking-l"
                    x1="16.5"
                    x2="15.5"
                    y1="7"
                    y2="14"
                    gradientUnits="userSpaceOnUse"
                >
                    <stop stopColor="#553B3E"></stop>
                    <stop offset="1" stopColor="#3D2432"></stop>
                </linearGradient>
                <linearGradient
                    id="thinking-q"
                    x1="12.5"
                    x2="14"
                    y1="24"
                    y2="25.5"
                    gradientUnits="userSpaceOnUse"
                >
                    <stop stopColor="#FA9428"></stop>
                    <stop offset="1" stopColor="#FA9428" stopOpacity="0"></stop>
                </linearGradient>
                <linearGradient
                    id="thinking-r"
                    x1="11.5"
                    x2="13"
                    y1="28.5"
                    y2="30"
                    gradientUnits="userSpaceOnUse"
                >
                    <stop stopColor="#FA9428"></stop>
                    <stop offset="1" stopColor="#FA9428" stopOpacity="0"></stop>
                </linearGradient>
                <linearGradient
                    id="thinking-s"
                    x1="11.5"
                    x2="13"
                    y1="26.5"
                    y2="28"
                    gradientUnits="userSpaceOnUse"
                >
                    <stop stopColor="#FA9428"></stop>
                    <stop offset="1" stopColor="#FA9428" stopOpacity="0"></stop>
                </linearGradient>
                <filter
                    id="thinking-m"
                    width="14.533"
                    height="14.688"
                    x="2.468"
                    y="16"
                    colorInterpolationFilters="sRGB"
                    filterUnits="userSpaceOnUse"
                >
                    <feFlood
                        floodOpacity="0"
                        result="BackgroundImageFix"
                    ></feFlood>
                    <feBlend
                        in="SourceGraphic"
                        in2="BackgroundImageFix"
                        result="shape"
                    ></feBlend>
                    <feGaussianBlur
                        result="effect1_foregroundBlur_31_62"
                        stdDeviation="1"
                    ></feGaussianBlur>
                </filter>
                <filter
                    id="thinking-p"
                    width="10"
                    height="4"
                    x="8.5"
                    y="21.5"
                    colorInterpolationFilters="sRGB"
                    filterUnits="userSpaceOnUse"
                >
                    <feFlood
                        floodOpacity="0"
                        result="BackgroundImageFix"
                    ></feFlood>
                    <feBlend
                        in="SourceGraphic"
                        in2="BackgroundImageFix"
                        result="shape"
                    ></feBlend>
                    <feGaussianBlur
                        result="effect1_foregroundBlur_31_62"
                        stdDeviation="0.75"
                    ></feGaussianBlur>
                </filter>
            </defs>
        </svg>
    );
};

export { MoodThinking };
