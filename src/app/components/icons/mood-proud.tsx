type MoodProps = {
    size?: `size-${number}`;
};

const MoodProud = ({ size = "size-8" }: MoodProps) => {
    return (
        <svg
            className={size}
            xmlns="http://www.w3.org/2000/svg"
            fill="none"
            viewBox="0 0 32 32"
        >
            <g clipPath="url(#proud-a)">
                <path
                    fill="url(#proud-b)"
                    d="M15.999 29.998c9.334 0 13.999-6.268 13.999-14 0-7.73-4.665-13.998-14-13.998C6.665 2 2 8.268 2 15.999c0 7.731 4.664 13.999 13.999 13.999z"
                ></path>
                <path
                    fill="url(#proud-c)"
                    d="M15.999 29.998c9.334 0 13.999-6.268 13.999-14 0-7.73-4.665-13.998-14-13.998C6.665 2 2 8.268 2 15.999c0 7.731 4.664 13.999 13.999 13.999z"
                ></path>
                <path
                    fill="url(#proud-d)"
                    d="M15.999 29.998c9.334 0 13.999-6.268 13.999-14 0-7.73-4.665-13.998-14-13.998C6.665 2 2 8.268 2 15.999c0 7.731 4.664 13.999 13.999 13.999z"
                ></path>
                <path
                    fill="url(#proud-e)"
                    fillOpacity="0.6"
                    d="M15.999 29.998c9.334 0 13.999-6.268 13.999-14 0-7.73-4.665-13.998-14-13.998C6.665 2 2 8.268 2 15.999c0 7.731 4.664 13.999 13.999 13.999z"
                ></path>
                <path
                    fill="url(#proud-f)"
                    d="M15.999 29.998c9.334 0 13.999-6.268 13.999-14 0-7.73-4.665-13.998-14-13.998C6.665 2 2 8.268 2 15.999c0 7.731 4.664 13.999 13.999 13.999z"
                ></path>
                <path
                    fill="url(#proud-g)"
                    d="M15.999 29.998c9.334 0 13.999-6.268 13.999-14 0-7.73-4.665-13.998-14-13.998C6.665 2 2 8.268 2 15.999c0 7.731 4.664 13.999 13.999 13.999z"
                ></path>
                <path
                    fill="url(#proud-h)"
                    d="M15.999 29.998c9.334 0 13.999-6.268 13.999-14 0-7.73-4.665-13.998-14-13.998C6.665 2 2 8.268 2 15.999c0 7.731 4.664 13.999 13.999 13.999z"
                ></path>
                <path
                    fill="url(#proud-i)"
                    d="M15.999 29.998c9.334 0 13.999-6.268 13.999-14 0-7.73-4.665-13.998-14-13.998C6.665 2 2 8.268 2 15.999c0 7.731 4.664 13.999 13.999 13.999z"
                ></path>
                <path
                    fill="url(#proud-j)"
                    d="M7.5 23a6.5 6.5 0 100-13 6.5 6.5 0 000 13z"
                    opacity="0.5"
                ></path>
                <path
                    fill="url(#proud-k)"
                    d="M24.5 23a6.5 6.5 0 100-13 6.5 6.5 0 000 13z"
                    opacity="0.5"
                ></path>
                <g filter="url(#proud-l)" opacity="0.5">
                    <path
                        stroke="#9A4609"
                        strokeLinecap="round"
                        strokeWidth="2"
                        d="M7.5 12.5s.25-2 2.5-2 2.5 2 2.5 2"
                    ></path>
                </g>
                <g filter="url(#proud-m)" opacity="0.5">
                    <path
                        stroke="#9A4609"
                        strokeLinecap="round"
                        strokeWidth="2"
                        d="M18.5 12.5s.5-2 2.5-2 2.5 2 2.5 2"
                    ></path>
                </g>
                <path
                    stroke="#43273B"
                    strokeLinecap="round"
                    strokeLinejoin="round"
                    strokeWidth="2"
                    d="M8 12s.25-2 2.5-2 2.5 2 2.5 2"
                ></path>
                <g filter="url(#proud-n)" opacity="0.26">
                    <path
                        stroke="#fff"
                        strokeLinecap="round"
                        strokeLinejoin="round"
                        strokeWidth="0.75"
                        d="M8.25 11.75s.25-2 2.5-2 2.5 2 2.5 2"
                    ></path>
                </g>
                <path
                    stroke="#43273B"
                    strokeLinecap="round"
                    strokeLinejoin="round"
                    strokeWidth="2"
                    d="M19 12s.5-2 2.5-2 2.5 2 2.5 2"
                ></path>
                <g filter="url(#proud-o)" opacity="0.26">
                    <path
                        stroke="#fff"
                        strokeLinecap="round"
                        strokeLinejoin="round"
                        strokeWidth="0.75"
                        d="M19.25 11.75s.25-2 2.5-2 2.5 2 2.5 2"
                    ></path>
                </g>
                <path
                    stroke="url(#proud-p)"
                    strokeLinecap="round"
                    strokeWidth="2"
                    d="M10 19c.5.667 2.5 2.5 6 2.5s5.5-1.833 6-2.5"
                ></path>
            </g>
            <defs>
                <radialGradient
                    id="proud-b"
                    cx="0"
                    cy="0"
                    r="1"
                    gradientTransform="rotate(132.839 10.786 10.065) scale(37.5033)"
                    gradientUnits="userSpaceOnUse"
                >
                    <stop stopColor="#FFF478"></stop>
                    <stop offset="0.475" stopColor="#FFB02E"></stop>
                    <stop offset="1" stopColor="#F70A8D"></stop>
                </radialGradient>
                <radialGradient
                    id="proud-c"
                    cx="0"
                    cy="0"
                    r="1"
                    gradientTransform="rotate(131.878 10.74 10.193) scale(38.9487)"
                    gradientUnits="userSpaceOnUse"
                >
                    <stop stopColor="#FFF478"></stop>
                    <stop offset="0.475" stopColor="#FFB02E"></stop>
                    <stop offset="1" stopColor="#F70A8D"></stop>
                </radialGradient>
                <radialGradient
                    id="proud-d"
                    cx="0"
                    cy="0"
                    r="1"
                    gradientTransform="rotate(101.31 2.876 12.808) scale(17.8466 22.8581)"
                    gradientUnits="userSpaceOnUse"
                >
                    <stop
                        offset="0.788"
                        stopColor="#F59639"
                        stopOpacity="0"
                    ></stop>
                    <stop offset="0.973" stopColor="#FF7DCE"></stop>
                </radialGradient>
                <radialGradient
                    id="proud-e"
                    cx="0"
                    cy="0"
                    r="1"
                    gradientTransform="matrix(-29 29 -29 -29 18 14)"
                    gradientUnits="userSpaceOnUse"
                >
                    <stop offset="0.315" stopOpacity="0"></stop>
                    <stop offset="1"></stop>
                </radialGradient>
                <radialGradient
                    id="proud-f"
                    cx="0"
                    cy="0"
                    r="1"
                    gradientTransform="rotate(77.692 -2.555 18.434) scale(28.1469)"
                    gradientUnits="userSpaceOnUse"
                >
                    <stop
                        offset="0.508"
                        stopColor="#7D6133"
                        stopOpacity="0"
                    ></stop>
                    <stop offset="1" stopColor="#715B32"></stop>
                </radialGradient>
                <radialGradient
                    id="proud-g"
                    cx="0"
                    cy="0"
                    r="1"
                    gradientTransform="matrix(7.5 10.99996 -7.97335 5.4364 16.5 16.5)"
                    gradientUnits="userSpaceOnUse"
                >
                    <stop stopColor="#FFB849"></stop>
                    <stop offset="1" stopColor="#FFB847" stopOpacity="0"></stop>
                </radialGradient>
                <radialGradient
                    id="proud-h"
                    cx="0"
                    cy="0"
                    r="1"
                    gradientTransform="matrix(11.49998 2 -2 11.49998 20.5 18)"
                    gradientUnits="userSpaceOnUse"
                >
                    <stop stopColor="#FFA64B"></stop>
                    <stop
                        offset="0.9"
                        stopColor="#FFAE46"
                        stopOpacity="0"
                    ></stop>
                </radialGradient>
                <radialGradient
                    id="proud-i"
                    cx="0"
                    cy="0"
                    r="1"
                    gradientTransform="rotate(43.971 -9.827 29.173) scale(59.0529)"
                    gradientUnits="userSpaceOnUse"
                >
                    <stop offset="0.185" stopOpacity="0"></stop>
                    <stop offset="1" stopOpacity="0.4"></stop>
                </radialGradient>
                <radialGradient
                    id="proud-j"
                    cx="0"
                    cy="0"
                    r="1"
                    gradientTransform="translate(7.505 16.501) scale(6.49487)"
                    gradientUnits="userSpaceOnUse"
                >
                    <stop stopColor="#F70A8D" stopOpacity="0.59"></stop>
                    <stop offset="1" stopColor="#FF6DC6" stopOpacity="0"></stop>
                </radialGradient>
                <radialGradient
                    id="proud-k"
                    cx="0"
                    cy="0"
                    r="1"
                    gradientTransform="translate(24.505 16.501) scale(6.49487)"
                    gradientUnits="userSpaceOnUse"
                >
                    <stop stopColor="#F70A8D" stopOpacity="0.59"></stop>
                    <stop offset="1" stopColor="#FF6DC6" stopOpacity="0"></stop>
                </radialGradient>
                <radialGradient
                    id="proud-p"
                    cx="0"
                    cy="0"
                    r="1"
                    gradientTransform="matrix(0 6 -9.18387 0 16 17)"
                    gradientUnits="userSpaceOnUse"
                >
                    <stop offset="0.518" stopColor="#4E2553"></stop>
                    <stop offset="0.703" stopColor="#5B4852"></stop>
                    <stop offset="1" stopColor="#4E2553"></stop>
                    <stop offset="1" stopColor="#4E2553"></stop>
                </radialGradient>
                <filter
                    id="proud-l"
                    width="10"
                    height="7"
                    x="5"
                    y="8"
                    colorInterpolationFilters="sRGB"
                    filterUnits="userSpaceOnUse"
                >
                    <feFlood
                        floodOpacity="0"
                        result="BackgroundImageFix"
                    ></feFlood>
                    <feBlend
                        in="SourceGraphic"
                        in2="BackgroundImageFix"
                        result="shape"
                    ></feBlend>
                    <feGaussianBlur
                        result="effect1_foregroundBlur_23_436"
                        stdDeviation="0.75"
                    ></feGaussianBlur>
                </filter>
                <filter
                    id="proud-m"
                    width="10.001"
                    height="7"
                    x="16"
                    y="8"
                    colorInterpolationFilters="sRGB"
                    filterUnits="userSpaceOnUse"
                >
                    <feFlood
                        floodOpacity="0"
                        result="BackgroundImageFix"
                    ></feFlood>
                    <feBlend
                        in="SourceGraphic"
                        in2="BackgroundImageFix"
                        result="shape"
                    ></feBlend>
                    <feGaussianBlur
                        result="effect1_foregroundBlur_23_436"
                        stdDeviation="0.75"
                    ></feGaussianBlur>
                </filter>
                <filter
                    id="proud-n"
                    width="8.75"
                    height="5.75"
                    x="6.375"
                    y="7.875"
                    colorInterpolationFilters="sRGB"
                    filterUnits="userSpaceOnUse"
                >
                    <feFlood
                        floodOpacity="0"
                        result="BackgroundImageFix"
                    ></feFlood>
                    <feBlend
                        in="SourceGraphic"
                        in2="BackgroundImageFix"
                        result="shape"
                    ></feBlend>
                    <feGaussianBlur
                        result="effect1_foregroundBlur_23_436"
                        stdDeviation="0.75"
                    ></feGaussianBlur>
                </filter>
                <filter
                    id="proud-o"
                    width="8.75"
                    height="5.75"
                    x="17.375"
                    y="7.875"
                    colorInterpolationFilters="sRGB"
                    filterUnits="userSpaceOnUse"
                >
                    <feFlood
                        floodOpacity="0"
                        result="BackgroundImageFix"
                    ></feFlood>
                    <feBlend
                        in="SourceGraphic"
                        in2="BackgroundImageFix"
                        result="shape"
                    ></feBlend>
                    <feGaussianBlur
                        result="effect1_foregroundBlur_23_436"
                        stdDeviation="0.75"
                    ></feGaussianBlur>
                </filter>
                <clipPath id="proud-a">
                    <path fill="#fff" d="M0 0h32v32H0z"></path>
                </clipPath>
            </defs>
        </svg>
    );
};

export { MoodProud };
